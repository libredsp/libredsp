use crate::simulator::{Graph, Packet};
...
*/
pub fn simulate(graph: &mut Graph, simulation_steps: usize) -> Option<HashMap<usize, Vec<f64>>> {
    ...
    for _ in 0..simulation_steps {
        ...
        for &node_id in &order {
            ...
            let mut inputs: Vec<Packet> = parent_ids
                .iter()
                .filter(|p| outputs.contains_key(p))
                .map(|p| outputs.get(p).unwrap().clone())
                .collect();
        ...
        for id in 0..graph.get_number_of_nodes() {
            ...
            let inputs: Vec<Packet> = parent_ids
                .iter()
    ...
    /* Collect recorded output once, after all steps have run */
    let mut recordings = HashMap::new();
    for node_id in 0..graph.get_number_of_nodes() {
        if let Some(buf) = graph.get_node_mut(node_id).get_recorded_output() {
            recordings.insert(node_id, buf.to_vec());
        }
    }
    Some(recordings)
}

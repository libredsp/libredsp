use crate::filter_design::frequency_selective::analog_filter_design;
...
pub fn analog_to_digital_transform_iir_filter_design(
    ...
    match method {
        ...
        AnalogToDigitalTransformationDesignMethod::Chebyshev => {
            poles =
                analog_filter_design::get_chebyshev_i_poles(n, omega_c, chebyshev_epsilon_factor);
        }
    }
...
pub fn h_of_s(poles: &[Complex<f64>], omega_c: f64, filter_type: &FilterType) -> TransferFunction {
    ...
    match filter_type {
        ...
        FilterType::Highpass { .. } => transform_analog_lowpass_to_highpass(poles, omega_c),
...
fn transform_analog_lowpass_to_highpass(poles: &[Complex<f64>], omega_c: f64) -> TransferFunction {
    let hp_poles: Vec<Complex<f64>> = poles
        .iter()
        .map(|p| {
            let denom = p.re * p.re + p.im * p.im;
            Complex::new(
    ...
    let tmp: Vec<Vec<Complex<f64>>> = hp_poles
        .iter()
        .map(|p| vec![Complex::new(1.0, 0.0), Complex::new(-p.re, -p.im)])
        .collect();
    ...
    num = num.iter().map(|c| c / gain).collect();
